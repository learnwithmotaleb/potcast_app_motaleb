...
class PlaylistController extends GetxController {
  ...
  final PagingController<int, SearchPodcast> pagingController =
      PagingController(firstPageKey: 1);
  final PagingController<int, Playlist> playListController =
      PagingController(firstPageKey: 1);
  ...
  Future<void> getPodcast(int pageKey, String search) async {
    ...
    try {
      final response = await apiClient.get(
          url: ApiUrl.search(pageKey: pageKey.toString(), searchTerm: search),
          showResult: true);
  ...
  Future<void> getPlayList(int pageKey) async {
    ...
    try {
      final response = await apiClient.get(
          url: ApiUrl.playList(page: pageKey), showResult: true);
  ...
  void playlistAdd() async {
    try {
      ...
      var response = await apiClient.post(
          url: ApiUrl.playListCreate(), body: body, showResult: true);
  ...
  void playlistDelete(
    ...
    try {
      ...
      var response = await apiClient.delete(
          url: ApiUrl.playListDelete(id: id), showResult: true);
